...
router.post('/:weaponId', async (req, res) => {
	...
	try {
		...
		if (e instanceof errors.NotFoundError) {
			console.warn(`[${req.user.uid}]: Could not find resources to add loadout weapon (loadoutId: ${loadoutId} weaponId: ${weaponId})`, e)
...
router.delete('/:weaponId', async (req, res) => {
	...
	try {
		...
		if (e instanceof errors.NotFoundError) {
			console.warn(`[${req.user.uid}]: Could not find resources to delete loadout weapon (loadoutId: ${loadoutId} weaponId: ${weaponId})`, e)
			return res.status(404)
				.end()
		}
