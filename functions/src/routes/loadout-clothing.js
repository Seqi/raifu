...
router.post('/:clothingId', async (req, res) => {
	...
	if (!clothingId) {
		return res.status(400).send('Clothing id is required')
	}
	...
	try {
		...
		if (e instanceof errors.NotFoundError) {
			console.warn(
				`[${req.user.uid}]: Could not find resources to add loadout clothing (loadoutId: ${loadoutId} clothingId: ${clothingId})`,
				e
			)
...
router.delete('/:clothingId', async (req, res) => {
	...
	try {
		...
		if (e instanceof errors.NotFoundError) {
			console.warn(
				`[${req.user.uid}]: Could not find resources to delete loadout clothing (loadoutId: ${loadoutId} clothingId: ${clothingId})`,
				e
			)
			return res.status(404).end()
		}
