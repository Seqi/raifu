...
export default (loadoutId) => ({
	add: (weaponId) => new CloudFunction().path(`/loadouts/${loadoutId}/weapons/${weaponId}`).post().then(toEntity),
	delete: (weaponId) =>
		new CloudFunction().path(`/loadouts/${loadoutId}/weapons/${weaponId}`).delete().then(toEntity),
	weapon: (weaponId) => ({
